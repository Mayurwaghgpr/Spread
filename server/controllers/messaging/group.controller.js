...
export const createGroupConversation = async (req, res, next) => {
    ...
    if (!groupName && membersArr.length < 1) {
        return res.status(401).json({message:"Invalid group credentials,please fill up all required field"})
    }
...
export const removeAsGroupAdmin = async (req, res, next) => {
    ...
    try {
       const isAddmin = await Members.find({ wher: { memberId: currentUserId, memberType: 'admin' } })
        if (!isAddmin) {
           return res.status(401).json({message:'Your not allowed to demote admin to user,only admin and make demote a admin'}) 
        }
